...
## ⚡ Headline results (default config · 1,000 cities · 2,000 MC paths)
...
|---|---:|---:|---:|
| **Cost-minimized** | **76.8%** | $12,840/yr | — |
| Balanced | 52.2% | $16,575/yr | 79 |
| Experience-optimized | 33.1% | $20,020/yr | 72 |

Switching the routing objective moves survival **33.1% → 76.8%**. That single table is the thesis: the lever is cost, not polish.
...
| Buy & settle · Daya Bay | 13.5% | $164k | ✅ |
| Nomad · experience | 12.3% | $0 | — |
| Buy & settle · Chengdu | 7.3% | $227k | ✅ |

**Three honest findings the engine surfaces:** (1) healthcare alone roughly *halves* survival (frugal drops 76.8% rent-only → 35.8% once medical + tax are modeled); (2) buying a home late after drawdown wrecks liquid survival but floors net worth; (3) the robust play is frugal + offshore.
...
> ⭐ **`open outputs/timeline.html`** — scrub or ▶ Play across all 372 months; the faint dots are all 1,000 cities, the blue trail is your route footprint (covering 29 of 31 provinces), the badge is the lifecycle phase (above: Sep 2036, age 60 — **Expedition** phase, **Lhasa, Tibet**; $1,750/mo, $186k spent, $340k left).
...
A 6-year "pre-game" before the simulation starts: (a) **build the backend** — the 1,000-city digital twin + data pipeline (done — `npm run anchors`), and (b) **field-test logistics** on near-term trips (e.g. with parents); if a transit route is too complex, flag it *High-Entropy* and pull it from your 70s into your 50s.
...
It loads **1,000 real Chinese cities**, scores each on risk, walks a year-by-year path that respects your aging health, then runs **thousands of simulated futures** to estimate the odds your savings survive to 80. Same inputs → same outputs, every time (fully deterministic).
...
- **Why 1,000 cities?** Every prefecture seat (~290) plus county seats sampled round-robin across provinces, so the frontier (Tibet/Xinjiang/Inner Mongolia) is covered — not just the dense east. Scales toward ~2,800 counties by raising the target.
...
```bash
npm install
npm run anchors         # download GeoNames -> data/city_anchors.json (1,000 cities) [needs python3 + network]
...
├── data/
│   ├── city_anchors.json        # 1,000 cities: REAL geo + curated tags (from GeoNames)
...
The dataset is built by `npm run anchors` from GeoNames (every PPLC/PPLA/PPLA2 seat + PPLA3 county seats sampled **round-robin across provinces** → ~1,000 with geographic spread, so the frontier is covered, not just the dense east). To go further:
...
这不是旅行规划器，而是**带约束的人生轨迹优化器**。它加载 **1,000 座真实城市**（全部地级市 + 主要县城），对每座城市的医疗 / 气候 / 海拔风险打分，在**年龄分段生命周期矩阵**（边疆探险→文化腹地→舒适沿海）下逐年在加权城市图上行走，对 50 万美元资产组合做耦合**住房、医疗、税务**的蒙特卡洛消耗模拟，并输出可导入 `.ics` 的**逐月居住日历**。它回答那些决定一切的问题：

> **钱能撑到 80 岁吗？哪种人生策略存活率最高？每个月该待在哪里？**

## ⚡ 核心结论（默认配置 · 1,000 城 · 2000 条路径）
...
| **成本最小化** | **76.8%** | $12,840/年 | — |
| 平衡 | 52.2% | $16,575/年 | 79 |
| 体验优先 | 33.1% | $20,020/年 | 72 |

仅切换路由目标，存活率从 **33.1% → 76.8%**。杠杆是成本，不是精细度。
...
每年做**一次长途迁徙**进入该段**轮换的目标省份**，再就地紧凑探索，探索阶段**绝不重访**。30 年间路线覆盖 **31 省中的 29 个**、约 56 座不同城市——是全国级广覆盖，而非少数大城市。**季度舒适审计**会标记任何超出 18–25°C 的季度（完整操作模型见下文「30 年路线图」一节）。
...
**三个诚实发现：**（1）仅医疗一项就让存活率**腰斩**（节俭从仅租房 76.8% 降到 35.8%）；（2）消耗多年后才在 66 岁买房会摧毁流动存活率，但托住净资产；（3）稳健打法是节俭 + 离岸。
...
模拟开始前的 6 年"预备期"：（a）**搭建后端**——1,000 城数字孪生 + 数据管线（已完成 `npm run anchors`）；（b）在近期行程（如与父母同行）**实测物流**；若某条中转过于复杂，标记为「高熵」并从 70 多岁提前到 50 多岁。
...
- **为何 1,000 城？** 全部地级市（约 290）加按省份轮询采样的县城，使边疆（西藏/新疆/内蒙古）也被覆盖，而非只有密集东部；提高目标可扩展到 ~2800 县。
...
```bash
npm install
npm run anchors         # 下载 GeoNames -> data/city_anchors.json（1,000 城）[需 python3 + 网络]
...
数据由 `npm run anchors` 从 GeoNames 构建（全部地级市 + 县城**按省份轮询采样** → ~1,000，地理均匀，使边疆也被覆盖，而非只有密集东部）。继续扩展：
