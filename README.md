...
A constrained life-trajectory optimizer — *not* a travel planner. It loads **2,348 real cities** — every province capital, prefecture seat, and county seat GeoNames provides (near-complete national coverage vs the [MCA 2013 census](https://xzqh.mca.gov.cn/statistics/2013.html): **31/34 provinces, 251/333 prefecture-level, 2,065/2,853 county-level**), scores each on medical / climate / altitude risk, walks a weighted city graph year by year under an **age-band lifecycle matrix** (frontier expeditions → cultural interior → comfort coasts), runs a Monte Carlo drawdown of a US$500k portfolio coupled with **housing, healthcare, and tax**, and emits a **month-by-month residence calendar** you can import as `.ics`. It answers the questions that decide everything:
...
## ⚡ Headline results (default config · 2,348 cities · 2,000 MC paths)
...
> ⭐ **`open outputs/timeline.html`** — scrub or ▶ Play across all 372 months; the faint dots are all **2,348 cities** (the whole country, frontier included), the blue trail is your route footprint (26 of 31 provinces over 30 years), the badge is the lifecycle phase (above: Dec 2036, age 60 — **Expedition** phase, **Nyingchi, Tibet**; $1,250/mo, $151k spent, $382k left).
...
A 6-year "pre-game" before the simulation starts: (a) **build the backend** — the 2,348-city digital twin + data pipeline (done — `npm run anchors`), and (b) **field-test logistics** on near-term trips (e.g. with parents); if a transit route is too complex, flag it *High-Entropy* and pull it from your 70s into your 50s.
...
It loads **2,348 real Chinese cities**, scores each on risk, walks a year-by-year path that respects your aging health, then runs **thousands of simulated futures** to estimate the odds your savings survive to 80. Same inputs → same outputs, every time (fully deterministic).
...
- **Why 2,348 cities?** That's *every* province capital, prefecture seat, and county seat GeoNames lists — near-complete national coverage (31/34 provinces, 251/333 prefecture, 2,065/2,853 county per [MCA 2013](https://xzqh.mca.gov.cn/statistics/2013.html)). The ~788 missing counties are mostly urban districts (市辖区) GeoNames doesn't list as separate seats; reaching the full 2,853 needs the MCA/NBS registry.
...
```bash
npm install
npm run anchors         # download GeoNames -> data/city_anchors.json (2,348 cities) [needs python3 + network]
...
├── data/
│   ├── city_anchors.json        # 2,348 cities: REAL geo + curated tags (from GeoNames)
...
## 📈 Coverage & scaling toward the full 2,853 counties

The dataset is built by `npm run anchors` from GeoNames — **every** seat it lists: PPLC/PPLA (province capitals), PPLA2 (prefecture seats), PPLA3 (county seats) → **2,348 nodes** across all 31 mainland provinces. Versus the [MCA 2013 census](https://xzqh.mca.gov.cn/statistics/2013.html) (34 / 333 / 2,853): GeoNames covers **31/34 provinces, 251/333 prefecture-level, 2,065/2,853 county-level**. To close the gap:

1. The ~788 missing county-level divisions are mostly **urban districts (市辖区)** GeoNames omits as separate seats; ingest the MCA/NBS division registry (with centroids) to add them. The enricher and engine are size-agnostic.
...
这不是旅行规划器，而是**带约束的人生轨迹优化器**。它加载 **2,348 座真实城市**——GeoNames 收录的全部省会、地级市与县城（对照[民政部 2013 区划](https://xzqh.mca.gov.cn/statistics/2013.html)的 34/333/2853：覆盖 **31/34 省、251/333 地级、2065/2853 县级**），对每座城市的医疗 / 气候 / 海拔风险打分，在**年龄分段生命周期矩阵**（边疆探险→文化腹地→舒适沿海）下逐年在加权城市图上行走，对 50 万美元资产组合做耦合**住房、医疗、税务**的蒙特卡洛消耗模拟，并输出可导入 `.ics` 的**逐月居住日历**。它回答那些决定一切的问题：

> **钱能撑到 80 岁吗？哪种人生策略存活率最高？每个月该待在哪里？**

## ⚡ 核心结论（默认配置 · 2,348 城 · 2000 条路径）
...
每年做**一次长途迁徙**进入该段**轮换的目标省份**，再就地紧凑探索，探索阶段**绝不重访**。30 年间路线覆盖 **31 省中的 26 个**、约 59 座不同城市——是全国级广覆盖，而非少数大城市（地图则显示全部 2,348 座）。**季度舒适审计**会标记任何超出 18–25°C 的季度（完整操作模型见下文「30 年路线图」一节）。
...
模拟开始前的 6 年"预备期"：（a）**搭建后端**——2,348 城数字孪生 + 数据管线（已完成 `npm run anchors`）；（b）在近期行程（如与父母同行）**实测物流**；若某条中转过于复杂，标记为「高熵」并从 70 多岁提前到 50 多岁。
...
- **为何 2,348 城？** 这是 GeoNames 收录的全部省会、地级市与县城——近全国覆盖（对照[民政部 2013](https://xzqh.mca.gov.cn/statistics/2013.html)：31/34 省、251/333 地级、2065/2853 县级）。缺的约 788 个县级多为 GeoNames 未单列的市辖区；补齐全部 2853 需民政部/统计局区划库。
...
```bash
npm install
npm run anchors         # 下载 GeoNames -> data/city_anchors.json（2,348 城）[需 python3 + 网络]
...
数据由 `npm run anchors` 从 GeoNames 构建——收录其全部 PPLC/PPLA（省会）、PPLA2（地级）、PPLA3（县城）→ **2,348 节点**，覆盖全部 31 个大陆省份。对照[民政部 2013 区划](https://xzqh.mca.gov.cn/statistics/2013.html)（34/333/2853）：覆盖 **31/34 省、251/333 地级、2065/2853 县级**。补齐差距：

1. 缺的约 788 个县级多为 GeoNames 未单列的**市辖区**；接入民政部/统计局区划库（含中心点）可补齐。enricher 与引擎与规模无关。
